...
import { ArrowLeft, TrendingUp, Scale, Plus, X, Info, CheckCircle2, AlertTriangle, Activity, Bot } from 'lucide-react';
...
export const WeightMonitor: React.FC<WeightMonitorProps> = ({ onBack }) => {
  ...
  const status: HealthStatus = useMemo(() => {
    if (currentWeight >= IDEAL_MIN && currentWeight <= IDEAL_MAX) return 'healthy';
    ...
    switch (status) {
      case 'healthy':
        return {
          ...
          button: 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200 dark:bg-emerald-900/50 dark:text-emerald-300',
          stroke: '#10B981', // Emerald 500
          gradientId: 'colorWeightEmerald',
          badge: 'bg-white/60 text-emerald-800',
          label: 'İdeal Kilo ✅',
          tip: 'Harika gidiyor! Barnaby ideal aralıkta.'
        };
      case 'warning':
        return {
          ...
          button: 'bg-amber-100 text-amber-700 hover:bg-amber-200 dark:bg-amber-900/50 dark:text-amber-300',
      ...
      case 'critical':
        return {
          ...
          button: 'bg-rose-100 text-rose-700 hover:bg-rose-200 dark:bg-rose-900/50 dark:text-rose-300',
  ...
  const minVis = 25;
  const maxVis = 40;
  const sliderPos = Math.min(Math.max(((currentWeight - minVis) / (maxVis - minVis)) * 100, 0), 100);
  ...
  return (
    <div className="flex flex-col h-full bg-stone-50 dark:bg-black min-h-screen">
       ...
       <div className="p-4 flex items-center justify-between sticky top-0 z-20 bg-stone-50/80 dark:bg-black/80 backdrop-blur-md">
        ...
        <h1 className="text-lg font-bold text-gray-800 dark:text-white">Kilo Takibi</h1>
      ...
      <div className="flex-1 p-4 space-y-6 overflow-y-auto pb-24">
        ...
        <AnimatedWrapper className={`rounded-[2.5rem] p-6 border transition-colors duration-500 relative overflow-hidden ${theme.bg} ${theme.border}`}>
           {/* Abstract Glow Background */}
           <div className={`absolute -top-10 -right-10 w-40 h-40 rounded-full blur-3xl opacity-20 ${status === 'healthy' ? 'bg-emerald-400' : status === 'warning' ? 'bg-amber-400' : 'bg-rose-400'}`}></div>

           {/* Card Header: Info + Log Button */}
           <div className="flex justify-between items-start mb-6 relative z-10">
              <div className="flex items-center space-x-3">
                 <img src={CURRENT_PET.photoUrl} alt="Pet" className="w-12 h-12 rounded-full border-2 border-white dark:border-slate-800 shadow-sm object-cover" />
                 <div>
                    <h2 className={`font-black text-lg ${theme.text}`}>{CURRENT_PET.name}</h2>
                    <p className={`text-xs font-bold opacity-70 ${theme.text}`}>{CURRENT_PET.breed} • {CURRENT_PET.age} Yaş</p>
                 </div>
              </div>
           ...
           <div className="flex flex-col items-center justify-center mb-8 relative z-10">
              <div className="flex items-baseline">
                 <span className={`text-6xl font-black tracking-tighter ${theme.text}`}>{currentWeight}</span>
                 <span className={`text-xl font-bold ml-1 opacity-60 ${theme.text}`}>kg</span>
              </div>
              <div className={`mt-2 px-3 py-1 rounded-lg backdrop-blur-md font-bold text-xs flex items-center shadow-sm ${theme.badge}`}>
                 {status === 'healthy' && <CheckCircle2 className="w-3.5 h-3.5 mr-1.5" />}
                 {status === 'warning' && <AlertTriangle className="w-3.5 h-3.5 mr-1.5" />}
                 {status === 'critical' && <Activity className="w-3.5 h-3.5 mr-1.5" />}
                 {theme.label}
              </div>
           </div>
           ...
           <div className="relative z-10">
              {/* Labels */}
              <div className={`flex justify-between text-[10px] font-bold uppercase tracking-wider mb-2 opacity-60 ${theme.text}`}>
                 <span>Düşük</span>
                 <span>İdeal ({IDEAL_MIN}-{IDEAL_MAX})</span>
                 <span>Fazla</span>
              </div>
              ...
              <div className="h-3 w-full bg-white/40 dark:bg-black/20 rounded-full overflow-hidden relative">
                 ...
                 <div className="absolute inset-0 w-full h-full opacity-80" 
                      style={{ background: 'linear-gradient(90deg, #F59E0B 0%, #10B981 30%, #10B981 60%, #F43F5E 100%)' }}>
                 </div>
              </div>
              ...
              >
                  <div className={`absolute -top-1.5 -left-1.5 w-4 h-4 bg-white dark:bg-slate-800 border-2 rounded-full shadow-md transition-colors duration-500 ${status === 'healthy' ? 'border-emerald-500' : status === 'warning' ? 'border-amber-500' : 'border-rose-500'}`}></div>
              </div>
           </div>
        </AnimatedWrapper>

        {/* 2. AI INSIGHT BUBBLE */}
        <AnimatedWrapper delay={100} className="flex items-start space-x-3 px-2">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm border ${theme.bg} ${theme.border}`}>
            ...
            <div className="relative bg-white dark:bg-slate-800 p-4 rounded-2xl rounded-tl-none shadow-soft border border-gray-100 dark:border-gray-700 flex-1">
                <p className="text-sm text-gray-600 dark:text-gray-300 font-medium leading-relaxed">
                   <span className="font-bold text-primary">PawPal AI:</span> {theme.tip}
                </p>
            </div>
        </AnimatedWrapper>

        {/* 3. HISTORY CHART (Subtle Data Viz) */}
        <AnimatedWrapper delay={200} className="bg-white dark:bg-slate-800 p-6 rounded-[2.5rem] shadow-soft border border-gray-100 dark:border-gray-700">
           <div className="flex justify-between items-center mb-6 px-2">
             <h3 className="font-bold text-gray-800 dark:text-white">Son 6 Ay</h3>
             <div className={`text-xs font-bold px-2 py-1 rounded-full ${isGain ? 'text-rose-500 bg-rose-50' : 'text-emerald-500 bg-emerald-50'}`}>
                {isGain ? '+' : ''}{diff}kg Değişim
             </div>
           </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-6 animate-in fade-in">
           <div className="bg-white dark:bg-slate-900 w-full max-w-xs rounded-[2rem] p-6 shadow-2xl relative animate-in zoom-in-95 duration-200 border border-gray-100 dark:border-gray-800">
              ...
              <h3 className="text-lg font-extrabold text-center mb-6 dark:text-white">Bugünkü Kilo</h3>
              ...
              <div className="flex items-center justify-center mb-8">
                <input 
                  ...
                  autoFocus
                />
                <span className="text-xl font-bold text-gray-400 ml-2 mt-4">kg</span>
              </div>
