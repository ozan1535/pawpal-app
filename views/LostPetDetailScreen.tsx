...
export const LostPetDetailScreen: React.FC<LostPetDetailScreenProps> = ({
  pet,
  onBack,
  onUpdateStatus,
}) => {
  ...
  const handleIsCurrentUserOwner = async () => {
    const user = await getCurrentUser();
    const isUserOwnsLostPet = user.id === pet.user_id;
    setIsOwner(isUserOwnsLostPet);
  };

  const handleCall = () => {
    window.location.href = `tel:+90${pet.contact.replace(/\D/g, "")}`;
  };

  useEffect(() => {
    handleIsCurrentUserOwner();
  }, []);
      ...
      <div className="flex-1 px-6 py-8 space-y-8 overflow-y-auto pb-32">
        ...
        {pet.reward && pet.is_found === false && (
          <div className="bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20 border-2 border-yellow-200 dark:border-yellow-700/50 p-5 rounded-2xl flex items-center shadow-sm">
            ...
            <div>
              <h3 className="font-black text-yellow-800 dark:text-yellow-400 text-lg">
                ÖDÜL: {pet.reward} TL
              </h3>
              <p className="text-yellow-700 dark:text-yellow-500 text-sm font-medium">
                Bulan kişiye verilecektir.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-stone-100 dark:border-slate-800 shadow-sm">
            <div className="flex items-center text-gray-400 mb-2">
              ...
              <span className="text-xs font-bold uppercase">Tasma Rengi</span>
            </div>
            <p className="font-bold text-gray-900 dark:text-white">
              {pet.leash_color || "Tasma yok"}
            </p>
        ...
        <div>
          <h3 className="font-black text-gray-900 dark:text-white text-lg mb-3 flex items-center">
            <Info className="w-5 h-5 mr-2 text-gray-400" /> İletişim
          </h3>
          <p className="text-gray-600 dark:text-gray-300 leading-relaxed font-medium">
            +90 {pet.contact}
          </p>
        ...
        {/* {pet.sightings.length > 0 && (
          <div>
            <h3 className="font-black text-gray-900 dark:text-white text-lg mb-4">
              Görülme Bildirimleri ({pet.sightings.length})
            </h3>
            <div className="space-y-3">
              {pet.sightings.map((sighting) => (
                <div
                  key={sighting.id}
                  className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-stone-100 dark:border-slate-800 flex items-center justify-between"
                >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 bg-white/90 dark:bg-black/90 backdrop-blur-lg border-t border-stone-100 dark:border-slate-800 z-20">
        {pet.user_id ? (
          ...
          <div className="space-y-3">
            {pet.is_found === false ? (
              ...
              <div className="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-2xl border border-green-100 dark:border-green-900/50">
                <h3 className="font-black text-green-600 dark:text-green-400 text-lg mb-1">
                  Harika Haber! 🎉
                </h3>
                <p className="text-green-600/80 dark:text-green-400/80 font-medium text-sm">
                  Bu ilan 24 saat sonra otomatik olarak arşivlenecektir.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="flex space-x-4">
            ...
            <InteractiveButton
              onClick={handleCall}
              className="flex-1 bg-red-600 text-white font-black text-lg py-4 rounded-2xl shadow-xl shadow-red-600/30 flex items-center justify-center"
            >
    ...
    <div className="absolute inset-0 z-50 flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[2rem] p-6 shadow-2xl scale-100 animate-in zoom-in-95 duration-200">
        <div className="flex justify-end">
          <button
            onClick={onClose}
            className="p-2 bg-stone-100 dark:bg-slate-800 rounded-full text-gray-500"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium">
            Dostumuzun bulunmasına çok sevindik. Nasıl bulduğunuzu paylaşır
            mısınız?
          </p>
        </div>
