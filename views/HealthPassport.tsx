...
import { getUniquePetWeights, getUserPets } from "@/lib/utils";
import { getWeightByPetId } from "@/services/supabase/pet_weights.service";
import { getVaccinesByPetId } from "@/services/supabase/vaccines.service";
...
export const HealthPassport: React.FC = () => {
  ...
  const { petId } = usePet();
  const getPetWeight = async () => {
    const pets = await getUserPets();
    const petWeights = await getWeightByPetId(/* pets[0].id */ petId);
    const petVaccineHistory = await getVaccinesByPetId(/* pets[0].id */ petId);
    const items = getUniquePetWeights(petWeights);
    const today = new Date().toISOString().split("T")[0];
    const pastVaccines = petVaccineHistory.filter(
      (v) => v.vaccine_date < today
    );
    setVaccines(pastVaccines);
    setData(
      items
        .map((item) => ({
  ...
  if (!data || !vaccines) return <LoadingScreen />;
  return (
    <div className="flex flex-col space-y-8 pb-24">
      {/* Header */}
      <div className="flex justify-between items-center px-2">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-800 dark:text-white transition-colors duration-300">
            Sağlık Pasaportu
          </h1>
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] shadow-soft border border-gray-100 dark:border-slate-700 transition-colors duration-300">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h3 className="font-bold text-gray-800 dark:text-white text-lg transition-colors duration-300">
              Kilo Takibi
            </h3>
      ...
      <div className="px-2">
        <h3 className="font-bold text-xl text-gray-800 dark:text-white mb-4 transition-colors duration-300">
          Tıbbi Geçmiş
        </h3>
        <div className="space-y-4">
          {vaccines.length > 0 ? (
            vaccines.map((record, index) => {
              const itemDate = new Date(record.vaccine_date).toLocaleDateString(
                "tr-TR",
                {
                  month: "short",
                  day: "numeric",
                  ...
                  <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-slate-700 flex items-center justify-between relative overflow-hidden transition-colors duration-300">
                    ...
                    <div className="flex items-center space-x-4 pl-2">
                      <div className="flex flex-col items-center justify-center bg-gray-100 dark:bg-slate-700 rounded-xl w-14 h-14 transition-colors duration-300">
                        <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase transition-colors duration-300">
                          {itemDate.split(" ")[0]}
                        </span>
                        <span className="text-lg font-extrabold text-gray-800 dark:text-white transition-colors duration-300">
                          {itemDate.split(" ")[1]}
                        </span>
                      </div>
                      <div>
                        <h4 className="font-bold text-gray-800 dark:text-white text-lg transition-colors duration-300">
                          {record.vaccine_name}
                        </h4>
                        <p className="text-sm text-gray-500 dark:text-gray-400 transition-colors duration-300">
                          {record.doctor_name}
            ...
            <div>Herhangi bir veriniz bulunmamaktadır</div>
          )}
        </div>
      </div>
    </div>
