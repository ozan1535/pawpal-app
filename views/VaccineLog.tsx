...
import {
  ...
  deleteVaccineById,
  getVaccinesByPetId,
} from "@/services/supabase/vaccines.service";
import { getUserPets, sortDateTodayClosest } from "@/lib/utils";
...
export const VaccineLog: React.FC<VaccineLogProps> = ({ onBack }) => {
  ...
  const getPetVaccineHistory = async () => {
    ...
    const sortedVaccines = sortDateTodayClosest(
      petVaccineHistory,
      "vaccine_date"
    );

    setHistory(sortedVaccines);
  };

  useEffect(() => {
    getPetVaccineHistory();
  }, [history]);
  ...
  const handleAddVaccine = async () => {
    ...
    const newVaccine = await addNewVaccine(
      /* pets[0].id, */ petId,
      vaccineName,
      vetName,
      date
    );
    //console.log(newVaccine, "newVaccinenewVaccinenewVaccine");
  ...
  const handleDelete = async () => {
    if (
      window.confirm(
        "Bu sağlık kaydını silmek istediğine emin misin? Bu işlem geri alınamaz."
      )
    ) {
      await deleteVaccineById(selectedRecordId);
        ...
        <div className="space-y-4">
          {history.length ? (
            history.map((record) => {
              ...
              const hasVaccineComplited =
                record.vaccine_date >= new Date().toISOString().split("T")[0];

              return (
                <div
                  key={record.id}
                  className="relative filter drop-shadow-sm group"
                >
                  <div className="bg-white dark:bg-slate-800 rounded-2xl flex overflow-hidden">
                    ...
                    <div className="flex-1 p-4 flex flex-col justify-center relative">
                      ...
                      <div className="flex items-center justify-between mt-1">
                        ...
                        >
                          {hasVaccineComplited ? "Beklemede" : "Tamamlandi"}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <div>
              Henüz kayıtlı bir aşı bulunmamaktadır. Lütfen aşı ekleyiniz.
            </div>
          )}
        </div>
      </div>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-t-[2.5rem] p-6 animate-in slide-in-from-bottom pb-10">
            ...
            <div className="space-y-3">
              <button
                onClick={() => {
                  setIsMenuOpen(false);
                  alert("Düzenleme özelliği sonraki güncellemede!");
                }}
