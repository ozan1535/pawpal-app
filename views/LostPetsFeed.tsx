...
export const LostPetsFeed: React.FC<LostPetsFeedProps> = ({ onBack }) => {
  ...
  const handleUpdateStatus = (id: string, status: 'ACTIVE' | 'FOUND' | 'ARCHIVED') => {
    setPets(currentPets =>
  ...
  const filteredPets = pets.filter(pet => {
    if (activeTab === 'ACTIVE') return pet.status === 'ACTIVE';
    return pet.status === 'FOUND'; // Show Found pets in the other tab
  });
  ...
  return (
    <div className="flex flex-col h-full bg-stone-50 dark:bg-black min-h-screen">
      ...
      <div className="p-4 flex items-center justify-between sticky top-0 bg-stone-50/90 dark:bg-black/90 backdrop-blur-md z-20 border-b border-gray-200 dark:border-gray-800">
        ...
        <h1 className="font-black text-lg dark:text-white text-red-500">KAYIP HAYVANLAR</h1>
      ...
      <div className="px-4 pt-4 pb-2 flex space-x-2">
        <button
          ...
          className={`flex-1 py-3 rounded-xl font-black text-sm transition-all ${activeTab === 'ACTIVE'
        ...
        <button
          ...
          className={`flex-1 py-3 rounded-xl font-black text-sm transition-all ${activeTab === 'FOUND'
      ...
      <div className="p-4 space-y-6 pb-24">
        {filteredPets.length === 0 && (
          <div className="text-center py-20 opacity-50">
            ...
            <p className="font-bold text-gray-500">Bu kategoride ilan bulunamadı.</p>
          </div>
        )}
        ...
        {filteredPets.map((pet) => (
          <div
            ...
            className={`bg-white dark:bg-slate-900 rounded-[2rem] overflow-hidden shadow-lg border-2 cursor-pointer transition-transform active:scale-95 ${pet.status === 'ACTIVE'
          ...
          >
            <div className="relative h-64">
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-6 pt-12">
                ...
                <p className="text-white/80 font-medium">{pet.breed} • {pet.distance} uzakta</p>
              </div>
            </div>

            <div className="p-6">
              <div className={`flex items-start mb-4 p-4 rounded-xl ${pet.status === 'ACTIVE' ? 'bg-red-50 dark:bg-red-900/10' : 'bg-green-50 dark:bg-green-900/10'
                }`}>
                <MapPin className={`w-5 h-5 mr-3 mt-0.5 ${pet.status === 'ACTIVE' ? 'text-red-500' : 'text-green-500'
                  }`} />
                <div>
                  <h4 className="font-bold text-gray-900 dark:text-white">Son Görülme</h4>
                  <p className="text-gray-600 dark:text-gray-300 text-sm">{pet.lastSeen}</p>
                </div>
              </div>
