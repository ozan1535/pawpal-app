...
import { addNewReminder } from "@/services/supabase/reminder.service";
...
export const Booking: React.FC<BookingProps> = ({ onBack }) => {
  ...
  const formattedDate = (date) => format(date, "d MMM, yyyy", { locale: tr });

  // State
  const [selectedType, setSelectedType] =
    useState<ReminderCategory>("Veteriner");
  ...
  return (
    <div className="flex flex-col h-full bg-stone-50 dark:bg-black min-h-screen">
      ...
      <div className="flex-1 overflow-y-auto pb-32 animate-fade-in-up px-6 pt-2">
        {/* 1. Category Grid */}
        <div className="mb-8">
          <h3 className="text-gray-800 dark:text-white font-bold text-lg mb-4">
            Hatırlatma Türü
          </h3>
          <div className="grid grid-cols-3 gap-3">
            {categories.map((cat) => (
              <InteractiveButton
                key={cat.id}
                onClick={() => setSelectedType(cat.name as ReminderCategory)}
                className={`py-4 rounded-2xl flex flex-col items-center justify-center space-y-2 transition-all duration-300 border-2 ${
              ...
              >
                <div
                  className={`w-10 h-10 rounded-full ${cat.bg} flex items-center justify-center`}
                >
                  <cat.icon className={`w-5 h-5 ${cat.color}`} />
                </div>
                <span className="font-bold text-[10px] text-gray-600 dark:text-gray-300">
                  {cat.name}
                </span>
              </InteractiveButton>
            ))}
        ...
        {/* 2. Note Input */}
        <div className="mb-8">
          <label className="block text-gray-800 dark:text-white font-bold text-lg mb-4">
            Not Ekle
          </label>
          <div className="bg-white dark:bg-slate-900 p-4 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 focus-within:border-primary transition-colors">
            <input
              ...
              placeholder={
                selectedType === "vet"
                  ? "Örn: Dr. Ahmet Bey, Kuduz Aşısı..."
                  : selectedType === "food"
                  ? "Örn: X Marka Mama al..."
                  : selectedType === "meds"
                  ? "Örn: Sabah akşam 1 tok karna..."
                  : "Hatırlatma notu..."
              }
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          {/* Date */}
          <div>
            <label className="block text-gray-500 text-xs font-bold mb-2 ml-2">
              TARİH
            </label>
            <InteractiveButton
          ...
          <div className="flex space-x-3 overflow-x-auto no-scrollbar pb-2">
            {alertOptions.map((opt) => (
              <button
                ...
                className={`flex-shrink-0 px-5 py-3 rounded-xl font-bold text-xs transition-all border-2 ${
                  alertOffset === opt.id
        ...
        <div className="mb-8">
          ...
          <div className="bg-gray-100 dark:bg-slate-900 p-1.5 rounded-2xl flex justify-between">
            {(["never", "daily", "weekly", "monthly"] as RepeatFrequency[]).map(
              (r) => (
                <button
                  key={r}
                  onClick={() => setRepeat(r)}
              ...
              {Array.from({ length: 31 }, (_, i) => i + 1).map((day) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    setDate(dateOnly); // e.g., "2025-12-29"
                    setShownDate(formattedDate(dateOnly));
                    //setDate(`${day} Eki, 2024`);
