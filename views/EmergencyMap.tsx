  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      <div className="relative z-10 p-4 pt-6">
        <Button variant="secondary" onClick={onBack} className="w-12 h-12 p-0 rounded-full flex items-center justify-center">
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white dark:bg-slate-900 rounded-t-[2.5rem] p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] z-20 max-h-[60vh] overflow-y-auto border-t-4 border-gray-100 dark:border-slate-800 transition-colors duration-300">
        ...
        <h2 className="text-xl font-heading text-gray-900 dark:text-white mb-2">Acil Veterinerler</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 font-bold">Yakınlarda {EMERGENCY_VETS.length} adet 7/24 açık klinik bulundu.</p>

        <div className="space-y-4">
          {EMERGENCY_VETS.map((vet) => (
            <Card key={vet.id} className="bg-gray-50 dark:bg-slate-800 border-2 border-gray-100 dark:border-slate-700 p-4">
              <div className="flex justify-between items-start mb-2">
                <div>
                  <h3 className="font-bold text-gray-900 dark:text-white text-lg">{vet.name}</h3>
                ...
                <div className="text-right">
                  <span className="block font-black text-lg text-primary">{vet.distance}</span>
                  <span className="text-xs text-gray-400 dark:text-gray-500 font-bold">uzakta</span>
                </div>
              </div>

              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 font-medium">{vet.address}</p>

              <div className="flex space-x-3">
                <Button variant="danger" className="flex-1 h-12" fullWidth>
                  <Phone className="w-4 h-4 mr-2" /> Ara
                </Button>
                <Button variant="secondary" className="flex-1 h-12" fullWidth>
