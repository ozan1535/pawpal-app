...
export const CalendarModal: React.FC<CalendarModalProps> = ({ onClose }) => {
    ...
    // Mock Events
    const events: Record<number, { type: 'vet' | 'groom' | 'meds', title: string }[]> = {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2.5rem] p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center justify-between mb-6 bg-gray-50 dark:bg-slate-800 p-2 rounded-2xl">
                    <button onClick={handlePrevMonth} className="p-2 hover:bg-white dark:hover:bg-slate-700 rounded-xl transition-colors">
                        <ChevronLeft className="w-5 h-5 dark:text-white" />
                    </button>
                    <span className="font-bold text-lg dark:text-white">
                        {monthNames[currentDate.getMonth()]} {currentDate.getFullYear()}
                    </span>
                    <button onClick={handleNextMonth} className="p-2 hover:bg-white dark:hover:bg-slate-700 rounded-xl transition-colors">
                ...
                <div className="grid grid-cols-7 gap-2 mb-6 text-center">
                    {['Pz', 'Pt', 'Sa', 'Ça', 'Pe', 'Cu', 'Ct'].map((d, i) => (
                        <span key={i} className="text-xs font-bold text-gray-400 mb-2">{d}</span>
                    ))}
                    ...
                    {selectedDate && events[selectedDate] ? (
                        <div className="space-y-3">
                            {events[selectedDate].map((event, idx) => (
                                <div key={idx} className="flex items-center bg-white dark:bg-slate-700 p-3 rounded-xl shadow-sm border border-gray-100 dark:border-gray-600">
                                    <div className={`w-3 h-3 rounded-full mr-3 ${event.type === 'vet' ? 'bg-orange-500' :
                                            event.type === 'meds' ? 'bg-green-500' : 'bg-blue-500'
                                        }`} />
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-gray-400">
                            <p className="text-sm font-medium">Bugün için planlanmış etkinlik yok.</p>
                        </div>
                    )}
                </div>
